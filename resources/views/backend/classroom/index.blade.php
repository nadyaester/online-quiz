@extends('backend.layouts.master')
      ...
      <div class="header-body">
        <div class="row align-items-center py-4">
          <div class="col-lg-6 col-7">
            <h6 class="h2 text-white d-inline-block mb-0">Classroom</h6>
  ...
  <div class="table-responsive">
    ...
    <table class="table align-items-center table-flush">
      ...
      <tbody>
          ...
          <tr>
            ...
            <td>
              <a href="{{route('classroom.edit', [$class->id])}}">
                <button class="btn btn-primary btn-sm">Edit</button>
              </a>
            </td>
            <td>
                <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#exampleModal{{$class->id}}">Delete</button>
            </form>
              <!-- Modal -->
              <div class="modal fade" id="exampleModal{{$class->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                  <form action="{{route('classroom.destroy',[$class->id])}}" method="post">@csrf
                  {{method_field('DELETE')}}
    ...
    <div class="container">
          {{$classrooms->links('backend.layouts.pagination')}}
  </div>
  </div>
</div>
</div>
